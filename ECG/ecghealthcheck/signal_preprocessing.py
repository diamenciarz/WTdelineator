...
def get_few_shot_data(
    ...
    for norm, abnorm in zip(normal_files, abnormal_files):
        norm_ecgs.append(
            scipy.io.loadmat(f"ECG/ecghealthcheck/data/{norm}")["ECG"][:, :ECG_LENGTH]
        )
        abnorm_ecgs.append(
            scipy.io.loadmat(f"ECG/ecghealthcheck/data/{abnorm}")["ECG"][:, :ECG_LENGTH]
        )
...
def normalize_ecg(ecg: np.ndarray) -> np.ndarray:
    for lead in range(ecg.shape[0]):
        ecg[lead] = (
            ecg[lead] - normalization_params["mean"][lead]
