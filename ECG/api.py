...
def convert_image_to_signal(image: Image.Image) -> np.ndarray or Failed:
    ...
    try:
        ...
        return Failed(
            reason="Failed to convert image to signal due to an internal error"
        )
...
def check_ST_elevation(
    ...
    explanation = (
        "ST elevation value in lead V3 ("
        + str(ste_mV)
        + " mV)"
        + (" did not exceed ", " exceeded ")[ste_bool]
        + "the threshold "
        + str(elevation_threshold)
        + ", therefore ST elevation was"
        + (" not detected.", " detected.")[ste_bool]
    )
    return (ste_assessment, TextExplanation(content=explanation))
    # except Exception as e:
    #     return Failed(
    #         reason="Failed to assess ST elevation due to an internal error", exception=e
    #     )


def check_ST_elevation_with_NN(
    signal: np.ndarray,
...
def evaluate_risk_markers(
    ...
    try:
        ...
        return Failed(
            reason="Failed to evaluate risk markers due to an internal error",
            exception=e,
        )
...
def diagnose_with_risk_markers(
    signal: np.ndarray, sampling_rate: int, tuned: bool = False
    ...
    try:
        ...
        explanation = (
            "Criterion value calculated as follows: "
            + formula
            + str(stemi_criterion)
...
def check_BER_with_NN(
    ...
    try:
        ...
        return Failed(
            reason="Failed to check for BER due to an internal error", exception=e
        )
...
def check_MI_with_NN(signal: np.ndarray) -> Tuple[bool, TextExplanation] or Failed:
    ...
    try:
        ...
        return Failed(
            reason="Failed to check for MI due to an internal error", exception=e
        )


def check_ecg_is_normal(
    signal: np.ndarray, data_type: ECGClass
...
def get_qrs_complex(
    ...
    Args:
        signal (np.ndarray): array representation of ECG signal
                             (contains n rows, i-th row for i-th lead)
        sampling_rate (int): sampling rate (see readme for the requirements)
    ...
    # except Exception as e:
    #     return Failed(reason="Failed to get QRS complex peaks", exception=e)


def show_channel_qrs_complex(
    signal: np.ndarray, qrs_peaks: QRS_COMPLEX, channel: int
...
def show_full_qrs_complex(
    signal: np.ndarray,
    qrs_peaks: List[Dict[str, List[int]]],
    ...
    Args:
        ...
        qrs_peaks (List[Dict[str, List[int]]]): list of per-channel dictionaries mapping
            QRS-complex waves to their peaks coordinates in the channel
        channel_names (List[str], optional): list of channel names. If None, auto-generates names.
    ...
    """
    try:
        return qrs.show_full_qrs(signal, qrs_peaks, channel_names)
    except Exception as e:
        return Failed(reason="Failed to display QRS-complex", exception=e)
